...
COMPONENTS = [
    ...
    # NOTE: "davs", "dfs", "extch", "bkofc-summary" are NOT here --
...
def verify_patch(path: Path):
    ...
    has_marker = any(NEW_BLOCK_MARKER in l for l in lines)
...
def process_component(component_name: str, base_dir: Path,
    ...
    if verified:
        print(f"[OK] {component_name} patched successfully. {detail}")
        return "PATCHED_VERIFIED"
    else:
        print(f"[ERROR] {component_name} written but {detail}")
...
def main():
    ...
    for name, status in results.items():
        ...
        marker = "✔" if status == "PATCHED_VERIFIED" else (
            "✖" if status in ("PATCHED_VERIFY_FAILED", "SYNTAX_CHECK_FAILED") else "-"
        )
        print(f"  {marker} {name}: {label}")
        if status in ("PATCHED_VERIFIED", "PATCHED_VERIFY_FAILED"):
            changed_count += 1
        if status == "PATCHED_VERIFIED":
            verified_count += 1
        if status in ("PATCHED_VERIFY_FAILED", "SYNTAX_CHECK_FAILED", "FILE_NOT_FOUND"):
            failed_count += 1
