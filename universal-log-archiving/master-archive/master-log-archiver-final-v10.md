...
recover_archive() {
    ...
    #-------------------------------------------------------
    # Recovery Verification (at destination)
    #-------------------------------------------------------

    if ! verify_archive "$DEST_DIR/$ARCHIVE_NAME" "$FILES_ARRAY"; then

        log_error "Recovery verification failed after move."
        log_error "Source files will NOT be deleted."

        local CORRUPT_ARCHIVE
        CORRUPT_ARCHIVE="${DEST_DIR}/${ARCHIVE_NAME}.corrupt.$(date '+%Y%m%d_%H%M%S')"

        if ! mv -f "$DEST_DIR/$ARCHIVE_NAME" "$CORRUPT_ARCHIVE"; then
            log_error "Failed to quarantine invalid recovery archive: $DEST_DIR/$ARCHIVE_NAME"
        else
...
create_archive() {
    ...
    #-------------------------------------------------------
    # Verify Archive (at destination)
    #-------------------------------------------------------

    if ! verify_archive "$DEST_DIR/$ARCHIVE_NAME" "$FILES_ARRAY"; then

        log_error "Archive verification failed after move."
        log_info "Source files will NOT be deleted."

        if ! rm -f "$DEST_DIR/$ARCHIVE_NAME"; then
            log_error "Failed to remove invalid archive from destination: $DEST_DIR/$ARCHIVE_NAME"
        else
            log_warning "Invalid archive removed from destination: $DEST_DIR/$ARCHIVE_NAME"
        fi
...
print_execution_summary()
{
    ...
    echo "----------------------------------------------------------------------------------"
    printf "%-31s: %17s\n" "Total Components Count"                 "$TOTAL_COMPONENTS"
    ...
    printf "%-31s %17s\n" "Total Archives Already Existing"         "$ARCHIVES_EXISTING"
    echo
    ...
    if [[ "$FAILED_COMPONENTS" -gt 0 ]]; then
        echo
        printf "%-20s: %s\n" "Failed Components" "${FAILED_COMPONENTS_LIST%, }"
    fi
}
            ...
            "columns": [
              ...
              },
              {
                ...
                "items": [
                  ...
                  { "type": "TextBlock", "text": "Already Archived Existing", "isSubtle": true, "spacing": "Medium", "horizontalAlignment": "Center", "wrap": true }
                ]
              },
              {
                "type": "Column",
...
if ! send_notification "$ADAPTIVE_CARD"; then
    ...
    echo
    log_info "Teams notification sends successfully."
fi
